...
#                             by: Bri Hernandez                            #
#                                                                          #
#   Derived SPEI metrics from the NPP sites on the Joranda Basin using     #
...
getwd()
#setwd("C:/Users/bnher/OneDrive/Pictures/Desktop/EDI_Fellow/JornadaBasinLTER")
...
#source('climate-metrics/R/get_data.R')
source('R/get_data.R')
...
monthly_derived <- function(df, sitename, latitude=32.6169){
  ...
  dfm <- df %>% mutate(
    ...
    summarize(tavg = mean(Air_TempC_Avg, na.rm=T), #note the NA remove
              ...
              tmax=mean(Air_TempC_Max, na.rm=T)) # not the NA remove
  ...
  dfm <- dfm %>% mutate_at(vars(tavg,rhavg,rhmin,rhmax,tmin,tmax),
                           ~replace(., is.nan(.), NA))
  # When the datalogger is down, and other values are NA, ppt is still zero
  # Fix this, change to NA
  dfm[is.na(dfm$tavg), 'ppt'] <- NA
  ...
  tryCatch({
    ...
    spei_12mo <- get_spei(cwdiff, scale=12, na.rm=TRUE, locname=sitename)  
