...
raw.wid <- tidyr::pivot_wider (raw, names_from=variable, values_from = value)
raw.wid
...
combine.df <-spei_out %>%
  dplyr::left_join(y=raw.df, by=c("stationid", "date", "station_name", "lat"))
combine.df
...
pet.app <- lapply(dat.pet, function(x) SPEI::thornthwaite(x$tavg, x$lat[1]))
...
pet.tho <- pet.sp %>%
  dplyr::rename("EL PASO AP"="PET_tho",
...
pet.long <- pet.tho %>%
  tidyr::pivot_longer(cols=c("EL PASO AP":"TULAROSA"),
...
pet.df <- dplyr::arrange(pet.long, station_name)
pet.df

# Add PET to original data frame with tavg and prcp by using station_name
dat.fin <- dplyr::bind_cols(combine.df, pet.df)
...
pdsi <- scPDSI::pdsi(dat.fin$prcp, dat.fin$pet_tho, start = 1911, sc = TRUE )
summary(pdsi)
...
readr::write_csv(final.df, 'data/chihuahuandesert_derived_climate_metrics_1911_2021.csv')
...
ggplot(final.df, aes(x=date))+
  geom_line(ggplot2::aes(y = spei_12mo), color = "blue") +
  geom_line(ggplot2::aes( y = sc_pdsi), color = "red")
